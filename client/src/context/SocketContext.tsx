...
const SocketProvider = ({ children }: { children: ReactNode }) => {
    const { users, setUsers, setStatus, setCurrentUser, currentUser } =
        useAppContext()
    ...
    const handleUserLeft = useCallback(
        ({ user }: { user: User }) => {
            ...
            setUsers(users.filter((u: RemoteUser) => u.userId !== user.userId))
        },
        [setUsers, users],
    )
    ...
    useEffect(() => {
        const handleConnect = () => {
            // Re-join the room on reconnection
    ...
    }, [socket, currentUser.roomId, currentUser.username])
