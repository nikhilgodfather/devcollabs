...
class CursorWidget extends WidgetType {
    ...
    toDOM() {
        ...
        cursor.title = this.user.username
        ...
        // Add username label only if user is actively typing
        if (this.user.typing) {
            ...
            if (effect.is(updateRemoteUsers)) {
                ...
                for (const user of users) {
                    ...
                    if (
                        ...
                        if (from < to) {
                            newDecorations.push(
                                createSelectionDecoration(user, from, to),
                            )
                        }
                    }
                    ...
                    if (user.cursorPosition >= 0) {
                        ...
                        newDecorations.push(
                            createCursorDecoration(user, cursorPos),
                        )
                    }
                }
